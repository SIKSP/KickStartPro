		...
		for( var intArrayIndex = 0; intArrayIndex < intArrayLength; intArrayIndex++ ){
			...
			if( strElement.indexOf(strSelectedPageAndLabel) >= 0 )
			{
				...
				if( strName.length > 0){
					...
					strLocateBy = "xpath";
					strLocateByValue = strxPath.replace("{CO}", ",");
				}
...
//========================================
	...
	try{
		...
		if( intSelectedActionIndex > -1){
			arrValues = Get_Select_Items( "hDescription_id" ).split(",");
			document.getElementById( "Description_id" ).value = arrValues[intSelectedActionIndex];

			arrValues = Get_Select_Items( "hParameters_id" ).split(",");
			strParameters = arrValues[intSelectedActionIndex]
			strParameters = js_ReplaceAll(strParameters, "{PIPE}", "|");
			strParameters = js_ReplaceAll(strParameters, "{COLON}", ":");
			...
			strExample = js_ReplaceAll(strExample, "{PIPE}", "|");
			strExample = js_ReplaceAll(strExample, "{COLON}", ":");
...
//==========================================
	...
	try{
		...
		var iArrLength = arrSteps.length;
		var varStep;
		var strAction, strDescription, strParameters, strControl_Required, strExample;
			...
			for( iArrIndex = 0; iArrIndex < iArrLength; iArrIndex++ ){
				...
				if( iArrIndex == 0 ){
					...
					if( varStep.length > 0 ){
						...
						strDescription = arrElement[2];
						strParameters = arrElement[3];
						strControl_Required = arrElement[4];
						strExample = arrElement[5];
